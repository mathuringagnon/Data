...
graph <- ggplot(data = MassiveData, aes(x = EVT_CLASS, y = ACRES)) + geom_bar(stat = "identity", width = 0.25)

graph <- graph + theme(axis.text.x = element_text(angle = 64, hjust = 1))

graph + 

```

what percentage of the US does each region make up and then what classes make up waht percentage of each region?

```{r}
#get a data frame for the number of acres for each region

acreRegions <- MassiveData[, c( 4, 24)]
...
RegionsClass <- MassiveData[, c(4, 20,24)]

#head(acreRegions)

#tail(acreRegions)
  
  pie <- PieDonut(MassiveData, aes(SUB_REGION, ACRES))
  pie
